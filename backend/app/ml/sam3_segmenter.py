...
class SAM3Segmenter:
    ...
    def initialize(self) -> None:
        ...
        try:
            ...
            try:
                ...
                if settings.ALLOW_BOX_MASK_FALLBACK:
                    print("SAM models not available - using explicit box-mask fallback")
                else:
                    raise RuntimeError(
                        "SAM models are not available and ALLOW_BOX_MASK_FALLBACK=false"
                    ) from e2
    ...
    def segment_boxes(
        ...
        if self._model_type == "none":
            if not settings.ALLOW_BOX_MASK_FALLBACK:
                return [
                    {
                        "mask": None,
                        "box": box,
