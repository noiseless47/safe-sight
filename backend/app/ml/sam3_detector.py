...
class SAM3Detector:
    ...
    def initialize(self):
        ...
        try:
            ...
            self._handle_unavailable_model(
                f"SAM 3 Transformers dependencies are not available: {e}", cause=e
            )
        except Exception as e:
            logger.error(f"Failed to load SAM 3 model: {e}")
            print(f"Failed to load SAM 3 model: {e}")
            self._handle_unavailable_model(f"Failed to load SAM 3 model: {e}", cause=e)
    ...
    def _handle_unavailable_model(self, message: str, cause: Exception | None = None):
        ...
        if settings.USE_MOCK_DETECTOR:
            ...
            print("Using explicit mock detector because USE_MOCK_DETECTOR=true")
            return

        if settings.FAIL_ON_MISSING_MODELS:
            raise RuntimeError(
                f"{message}. Install SAM dependencies/model weights or disable this detector."
            ) from cause

        logger.error("%s - SAM detector disabled", message)
    ...
    def detect(self, frame: np.ndarray) -> Dict[str, Any]:
        ...
        if not self._model_available:
            ...
            if settings.FAIL_ON_MISSING_MODELS:
                raise RuntimeError(
                    "SAM 3 detector is unavailable and mock detections are disabled."
                )
            return results
