param(
  [switch]$GreenLight,
  [string]$Data = "backend\configs\ppe_presence_yolo11.yaml",
...
$PrepScript = Join-Path $Root "scripts\prepare_ppe_presence_dataset.py"
...
$ProjectDir = Join-Path $Root "backend\runs\ppe_presence"
$TargetWeights = Join-Path $Root "backend\weights\ppe_detector\best.pt"
$PresenceDataset = Join-Path $Root "data\ppe_construction_presence"
...
if (-not (Test-Path -LiteralPath (Join-Path $PresenceDataset "summary.json"))) {
  Invoke-Checked -FilePath $Python -Arguments @($PrepScript)
}

if (-not $GreenLight) {
  Write-Host "Presence-only PPE training is prepared but not started."
  Write-Host "When you approve training, run:"
  Write-Host "  .\scripts\train_ppe_presence_yolo11.ps1 -GreenLight"
  Write-Host ""
  Write-Host "Laptop-safe fallback:"
  Write-Host "  .\scripts\train_ppe_presence_yolo11.ps1 -GreenLight -ImgSz 640 -Batch 2 -Workers 0 -CpuThreads 2"
  Write-Host ""
...
def main():
    ...
    model.train(
        ...
        name="construction_presence_yolo11",
        exist_ok=True,
        batch=$Batch,
        amp=True,
        workers=$Workers,
...
$BestWeights = Join-Path $ProjectDir "construction_presence_yolo11\weights\best.pt"
if (Test-Path -LiteralPath $BestWeights) {
  Copy-Item -LiteralPath $BestWeights -Destination $TargetWeights -Force
  Write-Host "Best presence-only PPE weights copied to backend\weights\ppe_detector\best.pt"
}
else {
